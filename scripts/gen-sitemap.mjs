/* Generate public/sitemap.xml from the canonical route list in src/lib/seo.js,
   ...
   Non-fatal by design: if anything throws in a CI/CD env, we log and exit 0 so
   the committed public/sitemap.xml is used instead of failing the whole build. */
import { writeFileSync } from 'node:fs'
import { resolve } from 'node:path'

try {
  const { ROUTES, abs } = await import('../src/lib/seo.js')
...
`
  ...
  console.warn('gen-sitemap skipped (using committed public/sitemap.xml):', e.message)
  process.exit(0)
}
